<template>
  <nav class="h-17.5 fixed top-0 left-0 w-full px-6 md:px-16 lg:px-24 xl:px-32 flex items-center justify-between z-50 shadow-[0px_4px_25px_0px_#0000000D] transition-all ease-in-out">
...
if (import.meta.client) {
  ...
  watch(y , (newValue) => {
    if (newValue > 10) {
      ...
      document.querySelector('nav')?.classList.remove(...transparentTokens)
    } else {
      document.querySelector('nav')?.classList.remove(...coloredTokens)
      document.querySelector('nav')?.classList.add(...transparentTokens)
    }
  })
}
